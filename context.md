...
## Last Updated
2026-03-19 | Added recurring-tasks infrastructure for non-dev scheduled tasks
...
- **`recurring-tasks/`**: shared runner + 3 task configs (private-content, private-materials, private-ranking) with flock-based locking, scoped permissions, Discord notifications, and crontab generator
...
- Recurring tasks need `.env` configured with RECURRING_TASKS_WEBHOOK and crontab installed via `generate-crontab.sh --install`
...
- **Recurring tasks:** `recurring-tasks/runner.sh` is the shared runner; task configs in `recurring-tasks/tasks/`, prompts in `recurring-tasks/prompts/`

## Active Branch
claude/recurring-tasks
