...
## Last Updated
2026-03-11 | Add no-em-dash writing rule and clean up all existing em dashes across agent.md and templates
...
- `progress.md` is a mandatory core instruction, and every commit must include a progress.md entry
...
- **`context.md` update frequency reduced**: only updated on final branch commit (before PR) or during Session Wrap-Up, not on every intermediate commit
...
- **No em dashes rule** added to Communication section and reinforced in Writing Style; all existing em dashes in agent.md and templates cleaned up
