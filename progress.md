...
- 2026-08-07 | experiment | xp-001 uncensored (Nick-approved) + reconciled with the mechanism thread. `experiments/score-shadow.py` time-bounds the recall join with THREE distinct bounds: per-session floor for recall (fair to the retriever), log-global-start for demand (a fact about the user; the stricter floor under-counts it and the rule ships on demand==0, so tightening it would bias toward shipping), unbounded for audit. Recall 12% -> 25%, both printed, plus a loud n<15 warning. NOT a rig change: fingerprint recomputed from its declared inputs = 54893c36fdbd, identical to all 232 records, so no record invalidated and no window restart. Reconciled: `hooks/memory-lazy-tier.selftest.sh` re-run independently, 13/13 pass and 152/152 exact replays, state OFF. DO NOT ENABLE the tier before 2026-08-19 -- demotion removes entries from MEMORY.md and the experiment's signal is "did a session open a memory file", so enabling drives demand to zero in the ship-favouring direction. cand_opened is 1, not 0 (9de66d1, 989ea44)

- 2026-08-07 | guidance | `guidance/measurement-windows.md`: align a coverage denominator to the collector's own log window. xp-001's hook read 100% single-turn / 52% multi-turn, written up as "fires on the first prompt, unreliably after" and as a sampling-validity threat; it was a censored denominator (the log begins at the rig-freeze archive point, the denominator counted whole transcripts, and long sessions are the ones that straddle it). Corrected per-prompt join across all 124 transcripts: 153/153 = 100%, both segments. A rotation you performed yourself is still left-censoring. Rig untouched, `experiments/` HEAD still ca005ed (1d5bce5, merged f9e9a1a; dup tail from propagate-learning removed in 3493818)
...
|------|------|-------------|
| 2026-08-05 | guidance | Document the three auth tiers and domain-wide SSO in `auth-basepath.md`: which surfaces sit on which tier, the root-identity design, the two preconditions that keep header-trust from being an auth bypass, and the load-bearing `OIDCUnAuthAction pass`. (`bd85eb6`) |
