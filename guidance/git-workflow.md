...
### GitHub's Email-Privacy Push Block Checks the Committer, Not Just the Author (2026-07-22)
GitHub's "Block command line pushes that expose my email" setting rejects a push (`push declined due to email privacy restrictions`) if ANY commit's **committer** email is the real personal email — not only the author email. Running `git -c user.email="<real-email>" rebase ...` stamps that email as the **committer** on every replayed commit even when the original authors stay correct, which silently makes the branch unpushable and surfaces as a confusing rejection unrelated to the actual diff.

**Fix:** re-stamp the committer to the GitHub-provided noreply address before pushing: `git -c user.name="<username>" -c user.email="<username>@users.noreply.github.com" rebase -f <upstream>` (the `-f` forces a re-commit even when already on the base). For plain commits on personal repos, default `user.email` to the noreply address rather than the real one. Hit reconciling a diverged personal-repo `main` (7 local commits) that used a real-gmail rebase and became unpushable until re-stamped.

### `claude-auto-merger`-Style Bots Can Auto-Create/Merge a PR on Push, Before Your Own `gh pr create` Runs (2026-07-23)
Some repos' webhook auto-merger fires on the **push** event, not on PR creation — it opens (and may immediately merge) a PR for a recognized branch pattern (e.g. `claude/doc-sync-<N>`) the instant the branch lands on the remote, ahead of any explicit `gh pr create` call later in the same session. If that later call still runs, it fails with `GraphQL: A pull request already exists for <owner>:<branch>` — which reads as a blocked/failed PR creation but the change may already be merged. **Don't treat that error as failure**: run `gh pr list --head <branch> --state all` first to check whether the PR already exists (and was already merged) before concluding the push failed. Also don't assume a fresh branch name is collision-safe — branch names get reused across runs (e.g. two different runs both named `claude/doc-sync-400`), so a new push can collide with a prior run's already-merged-and-deleted PR of the same name; check `--state all`, not just open PRs.
