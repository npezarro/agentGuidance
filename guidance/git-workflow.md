...
- **Never push commits to a branch after the auto-merger has merged it.** The auto-merger closes and deletes the branch on merge. Any commit pushed after that point is silently stranded — it lives on the remote branch but never lands on main. The failure mode is invisible: no error, the push succeeds, but the content is lost. Real incident (run #668): a doc fix (`7e38153`) was pushed to `claude/learnings-24x` after PR #241 was already merged; the stale "not yet configured" text from commit `a72f26c` stayed on main. **Rule:** If you discover work to add after opening a PR, open a fresh PR against main rather than committing to the already-open branch.
