...
7. **A guidance/code fix is only "applied" once its PR is MERGED, not once it's pushed to a branch.** Run #952 reported the ESSENTIAL Rule 1 ambiguity fix (commit `7a99a21`) as "applied" because it was pushed onto this repo's staged branch — but that branch sat unmerged, so no live session ever saw the fix. Supervisor run #44 (2026-07-17) caught this directly: it checked `origin/main`'s `guidance/ESSENTIAL.md` and found the fix absent, and flagged that the following run's apparent violation-rate improvement couldn't be attributed to a change that was never deployed. Worse, by the time run #963 checked, this repo's long-lived staging branch (PR #320, `claude/learnings-923-deploy-consolidated`) had drifted into a genuine `CONFLICTING`/`DIRTY` merge state against `main` (not GitHub's usual lazy `UNKNOWN`) because `main` picked up an unrelated same-paragraph edit via a different PR (#326) while this branch sat open — every commit staged onto it since has been invisible to production AND at growing risk of a silent conflict. **How to apply:** verify `gh pr view <n> --json state,mergedAt,mergeable,mergeStateStatus` before describing any staged fix as "applied," "resolved," or "live" in a run summary — pushed-to-a-branch and merged-to-main are different claims, and only the latter licenses "applied." If mergeable comes back `CONFLICTING`, fix the conflict (or flag it prominently) rather than piling more commits onto a branch that can no longer land cleanly.
