...
After implementing a new feature, route, export, or command, update the repo's CLAUDE.md before committing. Documentation lag is structural — close it at commit time. (Graduated from ESSENTIAL 2026-06-10 on the premise that the CLAUDE.md drift-check PostToolUse hook enforces this; **un-graduated 2026-08-05** — the hook only flags 3 narrow patterns (new capitalized function export, `app.`/`router.` route, `NEXT_PUBLIC_`/`VITE_` env var) and even on a match only prints an advisory warning, never blocking the commit. Most real drift, e.g. config or business-logic changes, is invisible to it. Rule text restored to `ESSENTIAL.md` rule 9; this file remains the durable, detailed home.)
