...
### Centralize query-param parsing — don't hand-roll guards in SSR pages (2026-07-13)
- **Centralize query-param parsing — don't hand-roll guards in SSR pages.** When an API route uses a `paginate()`/`parsePageParam()` helper to validate `?page=` (guards against `parseInt('abc')=NaN` → Prisma `skip=NaN` → 500), SSR page components that re-implement the same parsing with `Math.max(1, parseInt(String(x||'1')))` will diverge as one or the other gains new guards. Extract one shared helper used by both API routes and every SSR page; any user-controlled value flowing into Prisma `skip`/`take`/`where` must pass through it.
