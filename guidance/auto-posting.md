...
1. **wordpressPosts repo**: as a .md file in `~/repos/wordpressPosts/` for manual review before publishing.
2. **Discord**: as an embed in the `#claude-agent-logs` channel on the private Discord server.

Your response IS the markdown file and the Discord log entry. Write accordingly, because both audiences are human readers.
...
- **wordpressPosts repo**: full markdown, reviewed before publishing
...
- **Target ~3,500 chars for primary content.** Depth beyond that is fine (the .md file gets all of it), but the core narrative should fit in the embed window.
...
- The hook scripts (wordpressPosts and Discord) both perform pattern-based redaction as a safety net, but do not rely on them. Treat every response as potentially public.
...
- **Start with a descriptive heading.** Your first `##` heading becomes the file title. Make it specific and meaningful. Good: `## Propagating Claude Code Hooks to All 30 Repos`. Bad: `## What I Changed`, `## Summary`.
