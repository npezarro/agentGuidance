...
**Mandatory trigger for automated sessions (fix-checker, learning-agent, autonomousDev-private, autonomous-dev):** At the END of every automated session, call `propagate-learning.sh` unconditionally. Qualifying events: any error worked around, any assumption that proved wrong, any retry requiring a different approach, any service config that needed changing, any PR that needed rework. If you completed with zero surprises, a single no-op call still satisfies this rule — it is idempotent. This rule fires regardless of whether you received a correction.
...
**"Missing" is a conclusion, not an observation.** A file absent from your checkout and a file that was deleted look identical to `ls`. Before reporting anything as missing, deleted, broken, or dead, confirm you are looking at a current copy: `git ls-tree origin/main -- <path>` and `git log --all --diff-filter=D -- <path>` settle it in one command each. Same for an installed script, a vendored file, or a cached credential: check the source, not your local snapshot. (2026-08-01: reported `cf-purge.sh` deleted and patched two skills to warn about a dead path; the checkout was 51 commits behind and the script was on main the whole time. The wrong finding reached the user and two skills before it was caught.)
...
**A completed `propagate-learning.sh` call does not by itself satisfy this rule.** Rule 1's script handles routing a known learning to its destinations; this rule's check-existing/patch-or-add/commit-push sequence is a separate step that must still happen. (2026-08-05, ecosystem-supervisor run #56: session `score-20260805-075426-2426793` cleanly passed Rules 1 and 2 but skipped this rule's own postmortem step — an isolated automated-session failure mode distinct from the interactive-session pattern below.)
...
When an alert is noisy or annoying, the first question is "is it *true*?", not "how do I make it stop?" Before relaxing a threshold, widening a cooldown, muting, or rerouting any alert, verify with evidence (the logs, the actual metric) that it is a false positive. If it is firing correctly, fix the underlying cause — muting a true alarm hides the failure it was catching (a paused throttle-safe auth probe once turned a real outage into a multi-day silent one). Corollary — **resilience masks rot**: a scheduled healer (token refresh, data sync, deploy retry) can be failing 100% while redundancy keeps the end-state alive, so liveness checks look green. Monitor each healer's success *rate*, not just liveness, and alert on sustained 0% success of the primary mechanism even when the outcome still looks healthy (a token refresh was 0/36 for weeks, masked by a fallback path). Cross-cutting pattern + monitor template: KB `patterns/silent-healer-failure.md`.

## 9. Update CLAUDE.md When Behavior Changes (un-graduated 2026-08-05)
Graduated out 2026-06-10 on the premise that the CLAUDE.md drift-check PostToolUse hook enforces it. It doesn't: the hook (`~/.claude/settings.json`) only fires on a `git commit` Bash call, only flags a diff matching one of three narrow patterns (a new capitalized-name function export, a new `app.`/`router.` route, or a new `NEXT_PUBLIC_`/`VITE_` env var), and even on a match it only prints an advisory warning — it unconditionally `exit 0`s and never blocks the commit. It is blind to most real drift: config/business-logic changes, Next.js file-system routing, new PM2 processes. After any commit that changes a repo's behavior (not just its exported surface), check whether that repo's CLAUDE.md needs updating — treat the hook's silence as "not evaluated," not "compliant." Root cause confirmed by reading the hook source, not inferred from correlation; violation rate was degrading before this re-promotion (13.6% 24h vs 4.5% 7d as of 2026-08-05, concentrated in interactive travel-assistant sessions). Source: ecosystem-supervisor run #56 Action Item 1, first flagged at S192.
...
- ~~Update CLAUDE.md When Adding Features~~ — **un-graduated 2026-08-05, now rule 9 above.** The drift-check hook is advisory-only and never blocks; it was not the enforcement mechanism this graduation assumed.
