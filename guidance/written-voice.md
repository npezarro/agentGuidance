...
- **No decorative quotation marks in any user-facing writing.** Quotation marks are reserved for four uses only: (1) direct speech, (2) verbatim citation from a source, (3) introducing or critiquing a specific term (single use), (4) code or string literals. Any quoted phrase that fits none of these is dead weight — remove it. Decision rule: read the sentence without the marks; if the meaning is preserved, the marks go. Applies to all text written on Nick's behalf: cover letters, emails, Discord messages, Google Docs, LinkedIn posts, and application materials.
