...
**Date:** 2026-03-18
**Trigger:** `taskDiscovery.js` had `REPOS_JSON_PATH` hardcoded to `/home/user/repos/...` (WSL path). The bot runs on the VM where that path doesn't exist. The autonomous agent's discovery scan found no repos.
...
3. **Check re-export chains.** discord-bot has a pattern where `debate.js → claudeReply.js → executor.js` — new exports need to traverse the full chain.
