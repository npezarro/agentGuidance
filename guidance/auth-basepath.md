...
- **Middleware matcher must include basePath prefix (middleware.ts only)**: In Next.js 16 basePath apps that use the deprecated `middleware.ts` file, the `config.matcher` must use the full path including the basePath prefix — e.g., `["/<app>/api/auth/signin/:path*"]`, NOT just `["/api/auth/signin/:path*"]`. Without the prefix, Next.js never fires the middleware for those routes. Same fix required for `req.nextUrl.pathname.startsWith()` check in the middleware body. Caused a 4-commit debugging loop on shopper (2026-05-17).
- **proxy.ts uses UNPREFIXED matcher + stripped path**: When migrating to Next.js 16's preferred `proxy.ts` (replacing `middleware.ts`), the matcher must NOT include the basePath prefix — use `["/api/auth/signin/:path*"]`. Inside the proxy function, `req.nextUrl.pathname` is also the stripped (no-basePath) path, so use `.startsWith("/api/auth/signin/")`. Confirmed working on humans (2026-06-03, Next.js 16.2.7) by curl'ing the POST signin path and checking the `Set-Cookie: __auth_target=/<app>` header appears. Initial attempt used the prefixed matcher from the middleware.ts rule above — that silently failed (cookie never set), causing the auth-proxy to lose the routing target.
