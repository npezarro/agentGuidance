...
- **NextAuth `basePath` must always be `"/api/auth"`, never `"/<app>/api/auth"`**: In Next.js standalone output (`output: 'standalone'`), the basePath is stripped from `req.url` before the request reaches server code. NextAuth always receives `/api/auth/...` regardless of the configured Next.js basePath. If you set `basePath: "/<app>/api/auth"` in `auth.ts`, NextAuth cannot match any action routes and auth silently breaks. The template comment `// explicit, prevents AUTH_URL override` understates this — standalone stripping is the more critical reason. Setting the wrong value caused a debugging loop and revert on shopper (2026-05-18, commit 832c47b).
