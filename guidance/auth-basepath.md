...
1. **Action parsing**: the auth handler must receive a pathname that starts with the configured `basePath` -- @auth/core matches it with a `^`-anchored regex in `parseActionAndProviderId`. A mismatch throws `UnknownAction` and every `/api/auth/*` endpoint returns 400. **Next.js standalone has flip-flopped on whether it strips the app basePath first** (16.2.7 and older passed `/runeval/api/auth/...` straight through; 16.2.8+ strips it to `/api/auth/...`), so do not hardcode either assumption -- normalize the pathname in the route handler.

2. **Callback URL construction**: @auth/core derives the OAuth `redirect_uri` as `<origin><basePath>/callback/<provider>` (`parseProviders` does `new URL(config.basePath, url.origin)`). Under the auth-proxy design below there is exactly ONE registered redirect URI, `https://example.com/api/auth/callback/google`, so **`basePath` must stay the bare `/api/auth`** -- it is the proxy, not the redirect URI, that routes the callback to the right app. Adding the app subpath here makes Google reject the request with `Error 400: redirect_uri_mismatch`.

3. **AUTH_URL interference**: `next-auth`'s `setEnvDefaults()` extracts the pathname from `AUTH_URL` and uses it as `basePath` -- but only when `config.basePath` is falsy (`config.basePath || (config.basePath = pathname)`). Setting `basePath` explicitly wins, and @auth/core then uses only AUTH_URL's **origin** (`createActionURL` resets `url.pathname = "/"`, `reqWithEnvURL` substitutes the origin). An `AUTH_URL` that carries a path therefore logs `[auth][warn][env-url-basepath-mismatch]` while behaving correctly.
...
- **NextAuth `basePath` must always be `"/api/auth"`, never `"/<app>/api/auth"`** -- it breaks auth in TWO independent ways, and the second one bites even if you work around the first:
  1. *Action matching*: in standalone output the basePath is stripped from `req.url` before server code runs, so NextAuth receives `/api/auth/...`. A prefixed `basePath` fails the `^`-anchored match in `parseActionAndProviderId` and every `/api/auth/*` endpoint 400s with `UnknownAction`. Source: shopper debugging loop (2026-05-18, commit 832c47b). Caveat: standalone only started stripping in Next **16.2.8+** -- 16.2.7 and older passed the full path through, and that flip took runeval's OAuth down on 2026-07-09 (Dependabot PR #259). Normalize the pathname in the route handler so both behaviors work rather than depending on the current one.
  2. *`redirect_uri` construction*: @auth/core builds the OAuth redirect URI as `<origin><basePath>/callback/<provider>`. With one registered redirect URI per domain (the auth-proxy design), a prefixed basePath asks Google for an unregistered URI and sign-in dies with **`Error 400: redirect_uri_mismatch`** -- at Google, before any of your code runs. runeval hit exactly this on 2026-08-05 after commit `7c862ab` prefixed basePath purely to silence the benign `[auth][warn][env-url-basepath-mismatch]` log line. **Never change `basePath` to quiet that warning.**
  Check any app in one command: `curl -s https://example.com/<app>/api/auth/providers` must report `"callbackUrl":"https://example.com/api/auth/callback/google"` -- the bare origin, no app subpath.
