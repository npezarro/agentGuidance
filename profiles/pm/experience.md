...
---
## 2026-03-19 | autonomousDev scope and guardrails
**Task:** Define the scope and safety guardrails for the autonomous dev agent that runs unattended via cron.
**What worked:** Framed the scope as "fix-only, never feature": the agent should fix lint errors, failing tests, and small bugs, but never add new functionality or refactor code. This constraint made the risk assessment tractable: the worst case is a bad fix (revertible), not an unwanted feature (harder to remove). Added a protected repos list (centralDiscord, agentGuidance, autonomousDev) that the agent cannot touch.
