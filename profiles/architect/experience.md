...
---
## 2026-04-27 | trading-agent new data sources
**Task:** Plan implementation of 4 new data sources (estimate revisions, congressional trades, earnings NLP delta, short interest) for an autonomous trading agent.
**What worked:** Consolidating 3 of 4 sources on one API provider (FMP free tier, 250 calls/day) and budgeting calls upfront. Reading the full existing codebase first -- the consumed/unconsumed pattern, the research_reports catch-all table, and the template variable substitution in run.sh all constrained the design in useful ways. Phasing implementation by signal value (estimates first, then short interest for squeeze cross-reference) rather than by difficulty.
**What didn't:** Initial assumption that FMP had short interest data -- web search proved it does not. Had to pivot to FINRA API + yfinance fallback. Also initially planned sentence-transformers for transcript NLP, but the PyTorch dependency (2 GB) would 10x the venv on a 4 GB RAM VM. TF-IDF + Claude research trigger is the right weight class.
**Learned:** When planning multi-source data integrations: (1) verify API availability with actual research before committing to a provider, (2) budget shared rate limits across all consumers upfront with a concrete table, (3) match the computational weight of NLP tools to the deployment environment -- a 22 MB model that requires a 2 GB runtime is not a 22 MB dependency. For trading systems specifically, the consumed/unconsumed pattern with mark-after-prompt-build is a clean way to ensure data flows through exactly once without complex state machines.
