...
---
## 2026-05-14 | multi-repo security scan (valueSortify, manchu-translator, claude-token-tracker, claudeNet, claude-browser-agent, reddit-auto-hide, claude-usage-monitor)
**Task:** Full 13-phase security scan of 7 public repos covering secrets, IPs, SSH, home paths, env files, key files, gitignore gaps, env documentation, git history, npm audit, and CORS/debug configs
**What worked:** Sequential per-repo scanning (parallel bash calls still cancel). Cross-referencing env vars used in code vs .env.example coverage to find undocumented config (claudeNet missing CLAUDENET_TOKEN/CLAUDENET_URL, claude-browser-agent missing 3 cowork env vars). Correctly triaging false positives: manchu-translator "token" hit was a Manchu dictionary entry about bronze identification tokens; claude-token-tracker "token" hits were all LLM token counting; reddit-auto-hide accessToken reads are standard userscript session-reuse pattern.
**What didn't:** /tmp/security-scan/ directory was cleaned mid-session; had to re-locate repos in ~/repos/ and re-run some scans
**Learned:** claude-browser-agent CORS wildcard (Access-Control-Allow-Origin: *) is the most significant config finding -- server listens on 127.0.0.1 which limits network exposure, but CORS * combined with Bearer token auth means any page the user visits could make authenticated requests if the token is somehow leaked. reddit-auto-hide has zero .gitignore (single-file userscript repo) -- minimal risk but no safety net. All 7 repos have clean npm audits (0 vulns where lockfiles exist), consistent noreply git email, and no hardcoded secrets. claudeNet .env.example is well-structured but missing worker-specific env vars that are used in bin/claudenet-worker.js.

---
## 2026-05-15 | shopper (pre-release security audit)
**Task:** Full security audit of public-facing AI buying assistant (Next.js + Docker Claude CLI bridge + SQLite) before public launch
