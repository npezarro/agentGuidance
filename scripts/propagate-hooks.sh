...
for REPO in $REPOS; do
  ...
  # Copy .gitattributes (merge strategy for progress.md)
  if [ -f "$GITATTRIBUTES" ]; then
    if [ -f "${REPO_DIR}/.gitattributes" ]; then
      # Append our rules if not already present
      if ! grep -q "progress.md merge=union" "${REPO_DIR}/.gitattributes" 2>/dev/null; then
        cat "$GITATTRIBUTES" >> "${REPO_DIR}/.gitattributes"
      fi
    else
