...
- **Always push to GitHub.** When working on code or producing written materials (prep docs, application materials, notes), commit and push to the relevant repo. The remote is the source of truth — local-only files are invisible to other sessions and effectively don't exist. This applies to all work, not just code.
