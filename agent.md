...
- **Track job prep in the pipeline.** When producing application or prep materials for a role, always: (1) commit materials to `assortedLLMTasks/applications/`, (2) push to GitHub, (3) append the role to the Job Data tab in the Google Sheet via Apps Script (see `privateContext/infrastructure.md` for API details), and (4) include a direct GitHub link to the materials in the "Application Materials" column.
